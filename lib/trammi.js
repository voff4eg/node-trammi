...
function getStopInfo(stop, stopLine, cb) {
    request({
        uri: "http://gmmobile.atm-mi.it/wsbw/AttesaFermata",
        headers: {
            Cookie: "ASP.NET_SessionId=rrhu15qe3i3kt3iauepnuyqa",
        ...
        jQuery(".message tr", body).each(function (index, el) {
            var line, eta;

            line = jQuery("td:eq(0)", el).text().trim();
            eta = jQuery("td:eq(2)", el).text().trim();
