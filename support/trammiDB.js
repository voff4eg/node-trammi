...
function findStopNeighbors(stopLine, stopCode, stopDescr) {
    var i, n = stops.length, s, neighbors = [];

    for (i = 0; i < n; i += 1) {
        s = stops[i];
        if (s.sdescr === stopDescr &&
                s.scode !== stopCode &&
                s.lcode !== stopLine) {
            neighbors.push({
                scode: s.scode,
    ...
    for (l in lines) {
        if (lines.hasOwnProperty(l)) {
            ...
            for (i = 0; i < n; i += 1) {
                ...
                ng = findStopNeighbors(lines[l].line, s.scode, s.sdescr);

                m = ng.length;
                for (j = 0; j < m; j += 1) {
                    if (!isLinkAdded(net, s.scode, ng[j].scode, ng[j].lcode)) {
                        net[s.scode].push({
                            to: ng[j].scode,
                            d: 2,
                            l: ng[j].lcode
